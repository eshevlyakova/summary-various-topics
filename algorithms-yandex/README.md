...
Сериализация - это такой процесс, когда мы какую-то сложную структуру представляем в виде последовательности байтов или символов. **Сериализация дерева Хаффмана** - предназначен для сжатия информации. Идея в том, что используется часто, для того код короткий, что используется редко, для этого код подлиннее. Алгоритм построения дерева Хаффмана: каждый раз берем два самых редко встречающихся символа и объединяем их в один узел.
